/* ============================================================
   Barber Premium · VIZIO — bemestar.js
   Bem-estar & Pausas — réplica fiel do módulo do Inovar
   (padrão obrigatório INPERSON/VIZIO em todos os sistemas).
...
function bEsc(s){return (s==null?"":String(s)).replace(/</g,"&lt;");}
function renderBemEstar(){
  ...
  var linhas=ORD.map(function(k){var t=TIPOS[k];var on=p.tipos[k].on;
    return '<div style="display:flex;align-items:center;gap:14px;padding:14px 2px;border-bottom:1px solid var(--line)">'+
      ...
      '<div style="flex:1 1 auto;min-width:140px"><div style="font-weight:600">'+t.nome+'</div><div style="font-size:.82rem;color:var(--muted);line-height:1.4">'+bEsc(t.msgs[0])+'</div></div>'+
...
/* agendador ambiente */
reprograma();
if(!window._bpBemTimer)window._bpBemTimer=setInterval(tick,60000);
})();
