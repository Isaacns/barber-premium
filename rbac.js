/* ============================================================
   Barber Premium · VIZIO — rbac.js · Controle de Usuários & Acessos
   Padrão INPERSON/VIZIO: perfis com permissões por módulo,
   hierarquia (nível) e CRUD de usuários com atribuição de perfil.
   Persistência local (bp_rbac_v1); no go-live mapeia p/ RLS no backend.
   Depende de app.js (modal, closeModal, toast, confirmar, uid).
   ============================================================ */
(function(){
"use strict";
var RKEY="bp_rbac_v1";
var MODULOS=[["agenda","Agenda"],["clientes","Clientes"],["servicos","Serviços"],
  ["barbeiros","Barbeiros & Produtividade"],["estoque","Estoque"],["shop","SHOP"],
  ["financeiro","Financeiro"],["nfe","Nota Fiscal"],["dashboard","Dashboard Executivo"],
  ["alavancagem","Alavancagem"],["relatorios","Relatórios"],["bemestar","Bem-estar"],
  ["usuarios","Usuários & Acessos"],["identidade","Identidade / Camaleão"]];
...
function defaults(){
  return {
    perfis:[
      ...
      {id:"gerente",nome:"Gerente",nivel:2,desc:"Operação, gestão e relatórios; sem identidade e usuários.",
        perm:perm(['agenda','clientes','servicos','barbeiros','estoque','shop','financeiro','nfe','dashboard','alavancagem','relatorios','bemestar'],
                  ['agenda','clientes','servicos','barbeiros','estoque','shop','financeiro','nfe','alavancagem'])},
      {id:"financeiro",nome:"Financeiro",nivel:3,desc:"Financeiro, notas fiscais e relatórios.",
        perm:perm(['financeiro','nfe','dashboard','relatorios','clientes'],['financeiro','nfe'])},
      {id:"recepcao",nome:"Recepção",nivel:3,desc:"Agenda, clientes e SHOP (retirada).",
        perm:perm(['agenda','clientes','shop','dashboard','bemestar'],['agenda','clientes','shop'])},
      {id:"barbeiro",nome:"Barbeiro",nivel:4,desc:"Sua agenda, seus atendimentos e bem-estar.",
        perm:perm(['agenda','clientes','bemestar'],['agenda'])},
      {id:"estoque",nome:"Estoque",nivel:4,desc:"Insumos, produtos e compras.",
        perm:perm(['estoque','shop','dashboard'],['estoque'])},
      {id:"visualizador",nome:"Visualizador",nivel:5,desc:"Somente leitura.",
        perm:perm(['agenda','clientes','servicos','estoque','financeiro','dashboard','relatorios'],[])}
    ],
    usuarios:[
...
/* permissão do usuário logado (SESSION.perfil; default admin no piloto) */
window.rbacCan=function(mod,edit){ try{ var s=load();
  var pid=(window.SESSION&&SESSION.perfil)||'admin'; if(pid==='cliente')return false;
  var p=perfilById(s,pid);
...
function viewPerfis(s){
  ...
  return '<style>.rbSw{position:relative;display:inline-block;width:42px;height:24px}.rbSw input{display:none}'+
   ...
   '<div class="panel"><h3>Permissões por módulo — '+sel.nome+'</h3>'+
     ...
     '<div style="font-size:11.5px;color:var(--muted);margin-top:8px">"Editar" só vale com "Acesso" ligado. O perfil Administrador é protegido (acesso total). No go-live, estas permissões espelham as policies de RLS do backend.</div>'+
   '</div>';
}
...
window.rbacNovoPerfil=function(){
  modal("Novo perfil","",'<label>Nome do perfil</label><input id="rp_nome" placeholder="Ex.: Social media">'+
...
function viewUsuarios(s){
  ...
  return '<div class="panel"><div class="head"><h3>👥 Usuários</h3><div class="sp"></div>'+
     ...
     '<div style="font-size:11.5px;color:var(--muted);margin-top:8px">O convite/senha de novos usuários é gerado no go-live (auth real). Aqui você define nome, e-mail e perfil.</div>'+
   '</div>';
}
...
function formUser(u){ u=u||{}; var ed=!!u.id; var s=load();
  modal(ed?"Editar usuário":"Novo usuário","",'<label>Nome</label><input id="ru_nome" value="'+(u.nome||'').replace(/"/g,'&quot;')+'">'+
    '<label>E-mail</label><input id="ru_email" value="'+(u.email||'').replace(/"/g,'&quot;')+'" placeholder="nome@barbearia.com">'+
